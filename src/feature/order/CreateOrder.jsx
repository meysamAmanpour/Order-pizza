...
const formErrors= useActionData();
    ...
    <div className="py-6 px-4">
      <h2 className="mb-8 text-xl font-semibold">Ready to order? Let`&apos;s go! {username}</h2>
      ...
      <Form method="POST">
        ...
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-center   ">
          ...
          {formErrors?.phone && <p className="mt-2  text-xs font-semibold text-red-700 bg-red-100 p-2 rounded-md">{formErrors.phone}</p>}
          </div>
        </div>
...
export async function action({request}){
    ...
    const order={
        ...data,cart:JSON.parse(data.cart),priority:data.priority==="true"
    }
